...
namespace WebAPI_OTK.Controllers
{
    ...
    public class SalaryController : ControllerBase
    {
        ...
        public SalaryController(OtkDbContext context) => _context = context;
        ...
        public async Task<IActionResult> РассчитатьЗарплату([FromBody] РасчетЗарплатыDto параметры)
        {
            ...
            var операции = await _context.Операция_МЛ
                ...
                .Where(o => o.ДатаЗакрытия >= параметры.НачалоПериода &&
                            o.ДатаЗакрытия <= параметры.КонецПериода &&
                            o.Статус == "Завершена")
                .ToListAsync();
            ...
            foreach (var операция in операции)
            {
                ...
                // Базовая оплата за операцию (без учёта часов)
                var суммаОклад = ставка;
                var премия = 0m;
                ...
                {
                    if (операция.ДатаИсполнения.HasValue && 
                        (операция.ДатаИсполнения.Value.DayOfWeek is DayOfWeek.Saturday or DayOfWeek.Sunday))
                    {
        ...
        {
            var дата = операция.ДатаЗакрытия ?? операция.ДатаИсполнения ?? DateTime.Now;
            ...
            // Базовая премия за операцию
            return ставкаЧасовая * k.Коэффициент;
        }
