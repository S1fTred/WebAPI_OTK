        ...
        public async Task<ActionResult<КартаТехпланированияDto>> GetКартаТехпланирования(int id)
        {
                    ...
                    ДСЕ = i.ДСЕ.Select(d => new
                    {
                        ...
                        Операции = d.МЛ
                            ...
                            .Select(o => new
                            {
                                ...
                                o.ДатаНачала,
                                o.ДатаОкончания,
                                Длительность = o.ФактическаяДлительностьЧас
                            })
                            .ToList()
                    }).ToList()
                })
            ...
            var карта = new КартаТехпланированияDto
            {
                ...
                ДСЕ = изделие.ДСЕ.Select(d => new КартаДсеDto
                {
                    ...
                    Наименование = d.Наименование,
                    Операции = d.Операции
                        .GroupBy(x => new { x.ТипОперации, x.ДатаНачала, x.ДатаОкончания, x.Длительность })
                        .Select(g => new КартаОперацияDto
                        {
                            ТипОперации = g.Key.ТипОперации,
                            ДатаНачала = g.Key.ДатаНачала,
                            ДатаОкончания = g.Key.ДатаОкончания,
                            ДлительностьЧас = g.Key.Длительность
                        })
                        .ToList()
                }).ToList()
            };
